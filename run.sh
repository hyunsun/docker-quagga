...
sudo docker run --privileged --name $CONTAINER_HOSTNAME --hostname $CONTAINER_HOSTNAME -d -v ~/docker-quagga/volumes/quagga:/usr/etc quagga-fpm
sudo ~/docker-quagga/pipework $BRIDGE_NAME -i eth1 -l quagga $CONTAINER_HOSTNAME $CONTAINER_IP_CIDR
